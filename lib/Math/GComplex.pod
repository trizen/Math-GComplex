...
When the components of a complex number are native Perl numbers, the library handles the "division by zero" and the "logarithm of zero" cases.
However the user may still encounter incorrect results due to overflow or underflow in some special cases (such as C<coth(1e6) = (NaN NaN)>).
