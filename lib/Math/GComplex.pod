...
When the components of a complex number are native Perl numbers, the user may encounter errors due to division by zero or incorrect results due to overflow or underflow in some special cases.
