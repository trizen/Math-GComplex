...
The following functions are exportable:
    ...
    :misc
        cplx abs acmp sgn conj norm inv
        real imag reals floor ceil round
...
Raises C<$x> to power C<$n> and returns the result, where C<$n> is a native integer.

=head2 powmod

    $x->powmod($n, $m)

Modular exponentiation C<$x**$n % $m>, where C<$n> is a native integer.
...
Round C<$x> to the nearest Gaussian integer.

The half away from zero tie-breaking method is used, such that:
